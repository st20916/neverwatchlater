...
describe('PlaylistSetupPage 실 연동', () => {
  beforeEach(() => {
    global.fetch = vi.fn();
  });
  ...
  it('로그인 확인과 재생목록 설정이 모두 성공하면 완료 화면을 보여준다', async () => {
    ...
    expect(
      await screen.findByText(
        '‘Neverwatchlater’ 재생목록이 동기화 대상으로 지정되었습니다. (새로 생성됨)',
      ),
    ).toBeInTheDocument();
    expect(screen.getByText(/연결된 계정: 홍길동/)).toBeInTheDocument();
    expect(screen.queryByText(/user@example.com/)).not.toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: '정리 목록으로 이동' }),
    ).toBeInTheDocument();
  });
  ...
  it('로그인 세션이 없으면 로그인 확인 다이얼로그를 보여준다', async () => {
    ...
    expect(
      await screen.findByText('로그인이 필요합니다. 로그인 하시겠습니까?'),
    ).toBeInTheDocument();
  });
});
