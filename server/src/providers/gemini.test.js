...
test('summarizeTranscript는 Gemini 응답에서 3줄 요약 배열을 반환한다', async () => {
  let capturedUrl;
  let capturedBody;
  ...
  const result = await summarizeTranscript('자막 내용', { apiKey: 'key-1', model: 'gemini-test' });
  ...
  assert.ok(capturedBody.contents[0].parts[0].text.includes('자막 내용'));
});

test('summarizeTranscript는 apiKey가 없으면 즉시 에러를 던진다', async () => {
  globalThis.fetch = mock.fn(async () => {
    throw new Error('호출되면 안 됨');
  });

  await assert.rejects(
    () => summarizeTranscript('자막', { apiKey: '', model: 'gemini-test' }),
    /GEMINI_API_KEY/
  );
});

test('summarizeTranscript는 Gemini가 오류 응답을 주면 에러를 던진다', async () => {
  globalThis.fetch = mock.fn(async () =>
    geminiResponse(429, { error: { message: '쿼터를 초과했습니다.' } })
  );

  await assert.rejects(
    () => summarizeTranscript('자막', { apiKey: 'key', model: 'gemini-test' }),
    /쿼터를 초과했습니다/
  );
});

test('summarizeTranscript는 응답이 3개 문자열 배열이 아니면 에러를 던진다', async () => {
...
test('summarizeTranscript는 응답 텍스트가 JSON이 아니면 에러를 던진다', async () => {
  globalThis.fetch = mock.fn(async () =>
    geminiResponse(200, {
