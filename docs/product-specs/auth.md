...
- `POST /api/auth/logout` 호출 시 액세스 토큰을 `revokeToken`으로 폐기(best-effort)하고
  세션(`req.session`)을 삭제한 뒤, 브라우저의 `sid` 쿠키도 `clearCookie`로 제거한다.
  토큰 폐기가 실패해도 로그아웃 자체는 계속 진행한다.
- 세션이 만료되었거나 서버에 세션이 더 이상 없을 때(`GET /api/auth/me`, `requireAuth` 401)
  응답에서도 `sid` 쿠키를 지워, 브라우저에 잔여 쿠키가 남지 않게 한다.
